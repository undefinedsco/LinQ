...
参考微信桌面端的三栏式布局设计，适配 LinQ 的核心功能场景。
...
LinQ 使用标准的 8px 基准间距系统（8 Point Grid），所有间距都是 4px 的倍数。
...
LinQ 现在采用 Solid Protocol 官方配色方案，与 Solid Pod 生态保持一致的视觉品牌。所有颜色定义在 `apps/web/src/index.css` 中。
...
import { linqLayout } from '@/theme/spacing'
    ...
    <div className="h-screen w-screen bg-background text-foreground overflow-hidden">
      <ResizablePanelGroup direction="horizontal">
        ...
        <ResizablePanel 
          ...
          maxSize={15}      // 15% 最大宽度
          style={{ 
            minWidth: `${linqLayout.sidebar.minWidth}px`,      // 64px
            maxWidth: `${linqLayout.sidebar.maxWidth}px`       // 240px
          }}
        >
          <Sidebar 
        ...
        <ResizablePanel 
          ...
          maxSize={25}      // 25% 最大宽度
          style={{ 
            minWidth: `${linqLayout.listPanel.minWidth}px`,    // 180px
            maxWidth: `${linqLayout.listPanel.maxWidth}px`     // 400px
          }}
        >
          <ListPanel
            activeView={activeView}
        ...
        <ResizablePanel 
          minSize={50}      // 50% 最小宽度
          style={{ 
            minWidth: `${linqLayout.contentArea.minWidth}px`   // 400px
          }}
        >
          <ContentArea
            activeView={activeView}
...
3. **设计系统集成**：
   - 所有像素值来自 `@/theme/spacing.ts` 的 `linqLayout` 配置
   - 保持设计系统的一致性和可维护性
   - 支持主题切换和响应式调整
...
- [Solid Pod 集成文档](../specs/001-linq-hub/contracts/solid-pod-interactions.md)
- [LinQ 数据模型](../specs/001-linq-hub/data-model.md)
